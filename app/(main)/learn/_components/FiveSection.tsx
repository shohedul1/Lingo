...
const tabs = [
    ...
    },
    {
        ...
        subheading: 'web.sp currently supports English, Korean, Japanese, French, German, Spanish, and Portuguese. With more to come!',
        image: '/image.webp',
    },
    {
        ...
        images: [
            { title: 'Community meet-up in Tokyo', picture: '/slider1.jpg' },
            { title: 'Notion ambassador event in New York', picture: '/slider2.jpg' },
            { title: 'Sharing Notion setups in San Francisco', picture: '/slider3.jpg' },
            { title: 'Coffee pop-up in London', picture: '/slider4.jpg' },
            ...
            <div className="grid grid-cols-12 md:row-span-2 gap-4 xl:gap-6 mt-8 px-8 md:px-16 xl:px-0 xl:w-3/4 2xl:w-[55%] mx-auto md:w-full">
                {
                    tabs.map((tab, index) => (
                        <div key={index}
                            className={`col-span-12 ${index <= 2 ? 'md:col-span-6 lg:col-span-4 bg-[#c6a381] p-6 rounded-xl flex'
                            ...
                            {index <= 2 ? (
                                <div className="flex flex-col">
                                    <div className="text-5xl font-medium text-sky-600 text-center">
                                        {tab.text}
                                    </div>
                                    <div className="text-sm text-center">{tab.subtext} </div>
                                </div>
                            ) : index <= 4 ? (
                                <div>
                                            ...
                                            <div className="flex space-x-2 xl:space-x-6 ">
                                                {tab.images.map((image, index) => (
                                                    <div
                                                        ...
                                                        className={`${index === activeImageIndex ? 'p-2 rounded-full bg-[#eac32a]' : 'p-2 rounded-full bg-[#8afa2e]'}`}
                                                    >
